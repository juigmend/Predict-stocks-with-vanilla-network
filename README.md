...
The system collects stcoks' historical prices publicly available on the internet, from [Nasdaq's webpages for european stocks](https://www.nasdaq.com/european-market-activity/shares). 
...
The data is split into training and testing subsets. The training data is fed to a simple ("vanilla") artificial neural network composed by two hidden layers with ReLU activation. The output layer is only one node, with sigmoid activation. The network is trained iteratively with the backpropagation technique, until it reaches the minimum error. The error is calculated with the "binary cross entropy" formula, which is appropriate for sigmoid activation. Also, the network is initialised with Kaiming He's technique, which is appropriate for ReLU. 
